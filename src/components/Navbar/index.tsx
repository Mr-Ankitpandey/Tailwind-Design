  ...
  return (
    <div className='flex flex-col gap-8.5 px-4.5 py-5 lg:px-0 lg:py-0'   id='home'>
      <div className='flex justify-between items-center'>
        <div >
          <img className='w-43.75 lg:w-[239px]' src={Logo} alt='Logo' />
        </div>

        <div className='hidden lg:flex gap-8 lg:text-[25px] border-none'>
          <a href='#home'>Home</a>
          <a href='#shop'>Shop</a>
          <a href='#spa'>Spa</a>
      ...
      <div className='hidden md:flex justify-between'>
        <div className='flex gap-3'>
          {menuIcons.map((menu)=>(
            <Button className='py-2.5 px-3.75 bg-[#EFEFEF] md:text-[18px] lg:text-[25px] rounded-[5px]'>{menu}</Button>
          ))}
        </div>
        <div>
          <Button>
            <img src={SearchIcon} className="w-10 lg:w-[50px]" />
          </Button>
        </div>
      </div>
    </div>
